...
func (r *Runner) checkPrivilegedState(
	...
	if privilegedMode == NoopPrivilegedMigrations {
		...
		if privilegedHash := hashDefinitionIDs(privilegedDefinitionIDs); privilegedHash != expectedPrivilegedHash && up {
			...
			return errors.Newf("refusing to apply a privileged migration: re-run with the added flag `-privileged-hash=%s` to continue", privilegedHash)
		}

		if up {
			r.logger.Warn(
				"The migrator assumes that the following SQL queries have already been applied. Failure to have done so may cause the following operation to fail.",
				log.String("schema", schemaContext.schema.Name),
				log.String("sql", concatenateSQL(privilegedDefinitions, up)),
			)
		} else {
			r.logger.Warn(
				"The following SQL queries will need to be applied after the downgrade operation is complete.",
				log.String("schema", schemaContext.schema.Name),
				log.String("sql", concatenateSQL(privilegedDefinitions, up)),
			)
		}
